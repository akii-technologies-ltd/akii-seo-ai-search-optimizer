---
description: Get an Akii AI Visibility Score (0–100, 4-dimension breakdown) for any domain. Computed by an open-source LLM judge (Llama 4 / DeepSeek V4 Pro) against the brand's public footprint — same workflow as akii.com/ai-visibility-score, free. Not a direct query of ChatGPT / Claude / Gemini / Perplexity / Copilot.
...
2. **Fetch free model list**:
   ```bash
   curl -s -H "User-Agent: akii-plugin/2.6.0" \
...
3. **Start the scan**:
   ...
   curl -s -X POST https://akii.com/api/ai-visibility-score \
     ...
     -H "User-Agent: akii-plugin/2.6.0" \
     -d '<json-body>'
   ```
   Body:
   ```json
...
## Rules
- Always set `source: "plugin"` and `User-Agent: akii-plugin/2.6.0` so the request bypasses reCAPTCHA. Both required.
