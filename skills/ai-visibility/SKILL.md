...
```bash
curl -s -H "User-Agent: akii-plugin/2.6.12" https://akii.com/api/ai-visibility-score
```
...
curl -s -X POST https://akii.com/api/ai-visibility-score \
  ...
  -H "User-Agent: akii-plugin/2.6.12" \
  -d '{
    "brandDomain": "<domain>",
...
```bash
curl -s -H "User-Agent: akii-plugin/2.6.12" \
...
## Rules
- Always pass `source: "plugin"` AND `User-Agent: akii-plugin/2.6.12` for the API call — both required for reCAPTCHA bypass.
