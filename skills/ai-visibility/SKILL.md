...
```bash
curl -s -H "User-Agent: akii-plugin/2.6.0" https://akii.com/api/ai-visibility-score
```
...
## Rules
- Always pass `source: "plugin"` AND `User-Agent: akii-plugin/2.6.0` for the API call — both required for reCAPTCHA bypass.
